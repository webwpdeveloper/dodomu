<!-- Banner Full -->
<div class="section banner-section">
    <div class="banner full">

        <div class="banner-media">
            <picture>
                <!-- desktop -->
                <source srcset="img/desktop.webp" type="image/webp" media="(min-width:768px)">
                <source srcset="img/desktop.jpg" type="image/jpg" media="(min-width:768px)">
                <!-- mobile -->
                <source srcset="img/mobile.webp" type="image/webp" media="(max-width:767px)">
                <source srcset="img/mobile.jpg" type="image/jpg" media="(max-width:767px)">
                <img fetchpriority="high" width="380" height="240" src="#" alt="Banner Image" loading="eager">
            </picture>

            <!-- to display the video, add a class 'video-present' to the video -->
            <div class="video">
                <video muted playsinline loop preload poster="img/desktop.webp" src="#"
                    data-src="video/video.mp4"></video>
            </div>
        </div>


        <div class="banner-align align-bottom">
            <div class="container">
                <div class="row">
                    <div class="col-xl-7 ">
                        <div class="banner-info">
                            <h1 class="h1 title">
                                Software & product engineering
                            </h1>
                            <div class="text">
                                We understand the value of your time and money and identify the most effective approach
                                to meet your objectives.
                            </div>
                            <div class="banner-btn">
                                <a class="btn btn-secondary" href="about.php">
                                    <b>About Us</b>
                                </a>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>